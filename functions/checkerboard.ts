import type { WssParams } from "../types.d.ts";
import { formatPosition } from "../_utils.ts";
export default function checkerboard(
  { parameters, queueCommandRequest, state }: WssParams,
) {
  ...
  for (const position of positions) {
    ...
    queueCommandRequest(
      `setblock ${formatPosition(state, x, y, z, xOffset, 0, zOffset)} ${block}`,
    );
